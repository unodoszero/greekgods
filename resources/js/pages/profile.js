...
function formatWorkoutVolume(workout) {
    if (workout.workoutSets && workout.workoutReps) {
        const reps = workout.workoutRepsMax && workout.workoutRepsMax !== workout.workoutReps
            ? `${workout.workoutReps}–${workout.workoutRepsMax}`
            : workout.workoutReps;
        return `${workout.workoutSets} x ${reps}`;
    }
